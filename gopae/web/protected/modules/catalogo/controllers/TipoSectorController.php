...
class TipoSectorController extends Controller
{
    ...
    public static $_permissionControl = array(
        'read' => 'Consulta de TipoSectorController',
        'write' => 'Creación y Modificación de TipoSectorController',
        'admin' => 'Administración Completa  de TipoSectorController',
        'label' => 'Módulo de TipoSectorController'
    );
    ...
    {
        $model=new TipoSector('search');
        $model->unsetAttributes();  // clear any default values
        if($this->hasQuery('TipoSector')){
            $model->attributes=$this->getQuery('TipoSector');
        }
    ...
    public function actionRegistro()
    {
        ...
        if($this->hasPost('TipoSector'))
        {
            $model->attributes=$this->getPost('TipoSector');
            if($model->save()){
    ...
    {
        $model=TipoSector::model()->findByPk($id);
        if($model===null){
     ...
     * @param TipoSector $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if($this->hasPost('ajax') && $this->getPost('ajax')==='tipo-sector-form')
        {
    ...
    public function getActionButtons($data) {
        ...
        $columna .= CHtml::link("", "", array("class" => "fa icon-zoom-in", "title" => "Ver datos", 'href' => '/catalogo/tipoSector/consulta/id/'.$id)) . '&nbsp;&nbsp;';
        $columna .= CHtml::link("", "", array("class" => "fa icon-pencil green", "title" => "Editar datos", 'href' => '/catalogo/tipoSector/edicion/id/'.$id)) . '&nbsp;&nbsp;';
        $columna .= '</div>';
        return $columna;
    }
