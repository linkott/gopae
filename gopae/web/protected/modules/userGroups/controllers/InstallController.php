...
{ 	
	...
	private function createTables()
	{
		...
		catch(Exception $e)
		{
			...
			throw new CHttpException (500, Yii::t('userGroupsModule.install', 'Unable to create the tables in the database. Make sure that your db user has the necessary permissions to create a new table.<br/>This release only officially support Mysql and Postgres.'));
			return false;
		}
	}
	...
	private function initializeConfiguration()
	{
		...
		$configuration_model->value = 'TRUE';
		$configuration_model->options = 'BOOL';
		$configuration_model->description = 'users with just admin write permissions won\'t see the Main Configurations and Cron Jobs pannels';
		if (!$configuration_model->save())
			throw new CHttpException (500, Yii::t('userGroupsModule.install', 'Setting not installed. Installation abort.'));
		$configuration_model = new UserGroupsConfiguration('installation');
	...
	private function initializeGroups()
	{
		...
		$group_model->level = 1;
		//Linea agregada issue de google code		
