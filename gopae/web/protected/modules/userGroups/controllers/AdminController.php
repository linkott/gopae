...
/**
 * @author Nicola Puddu
 * @package userGroups accesslist actionadmin
 * admin controller
 */
class AdminController extends Controller
{
	...
	public static $_permissionControl = array(
            ...
            'label' => 'Herramienta de Administración de Usuarios',
        );
	...
	{
		if (Yii::app()->request->isAjaxRequest)
			$this->renderPartial('../../../../views/site/index', NULL, false, true);
		else
	...
	{	
		if ((int)$formData['id'] === UserGroupsUser::ROOT)
			Yii::app()->user->setFlash($formData['what'],Yii::t('userGroupsModule.admin','You cannot change the Access Permission of the Root User'));
			...
			if (Yii::app()->user->pbac('admin.admin')) {
				...
				if ($model->validate()) {
					if (!$model->save()) {
						Yii::app()->user->setFlash((int)$formData['what'] === UserGroupsAccess::USER ? 'user' : 'group', Yii::t('userGroupsModule.admin','A problem occurred during the Data and Access Permission saving action'));
	...
	private function itemDelete($formData)
	{
		...
		if (!Yii::app()->user->pbac('admin'))
			Yii::app()->user->setFlash((int)$formData['what'] === UserGroupsAccess::USER ? 'user' : 'group', Yii::t('userGroupsModule.admin','You don\'t have the permissions to delete any user/group'));
		else {
			...
			if(Yii::app()->request->isPostRequest && $formData['id'] !== 'new' && (int)$formData['id'] !== UserGroupsUser::ROOT) {
				...
				if ($model) {
					...
					if ($model->level < Yii::app()->user->level) {
						...
						Yii::app()->user->setFlash((int)$formData['what'] === UserGroupsAccess::USER ? 'user' : 'group', Yii::t('userGroupsModule.admin','You cannot delete a user/group with an higher level then yours.'));
				} else
	...
	private function configurationSave($formData)
	{
		if (Yii::app()->user->pbac('userGroups.admin.admin')) {
			...
			if ($successes == count($formData))
				...
				Yii::app()->user->setFlash('configuration', Yii::t('userGroupsModule.admin','A problem occurred during the configuration saving action'));
		} else
	...
	private function cronSave($formData)
	{
		if (Yii::app()->user->pbac('userGroups.admin.admin')) {
			...
			if ($successes == count($formData))
				Yii::app()->user->setFlash('crons', Yii::t('userGroupsModule.admin','Cron Jobs Configurations Saved'));
			else
				Yii::app()->user->setFlash('crons', Yii::t('userGroupsModule.admin','A problem occurred during the cron jobs configuration saving action'));
		} else
	...
	private function cronRemove()
	{
		...
		if (Yii::app()->user->pbac('userGroups.admin.admin')) {
			...
			Yii::app()->user->setFlash('crons', Yii::t('userGroupsModule.admin','Cron Jobs succefully removed'));
		} else
