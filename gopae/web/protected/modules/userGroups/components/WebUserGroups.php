...
class WebUserGroups extends CWebUser
{
	...
	private $apellido;
        ...
        /**
	 * @var string $estado the State Name of the User
	 */
	private $estado;
        
        /**
	 * @var integer $estadoId the State Id of the User
	 */
	private $estadoName;
	 ...
	 */
	protected function UGChangeIdentity($id,$name, $groups, $groupName, $level, $accessRules,$home,$recovery,$profile,$states, $unidadesResp=null)
	{
	    $this->setId($id);
	    $this->setName($name);
	    ...
	    $this->loadIdentityStates($states);
            $this->setUnidadesResp($unidadesResp);
	}
	
	
	protected function profileLoad($identity)
	{
            $this->cedula = $identity->cedula;
            $this->email = $identity->email;
            $this->nombre = $identity->nombre;
            $this->apellido = $identity->apellido;
            $this->estado = $identity->estado;
            $this->estadoName = $identity->estadoName;
            $this->unidadesResp = $identity->unidadesResp;
	...
	public function login($identity,$duration=0)
	{
	    ...
	    $states=$identity->getPersistentStates();
            ...
            if($this->beforeLogin($id,$states,false, $identity))
	    {
	        $this->UGChangeIdentity($id,$identity->getName(), $identity->getGroup(), $identity->getGroupName(), $identity->getLevel(), 
	        	$identity->getAccessRules(), $identity->getHome(), $identity->getRecovery(), $identity->getProfile(), $states, Yii::app()->session->get('unidadesResp'));
	        
	        $this->profileLoad($identity);
                
                Yii::app()->session->add('user', $this);
	...
	private function returnPermissionResult($request, $current_module, $current_controller)
	{
        ...
        public function getUnidadesResp() {
            return $this->unidadesResp;
        }

        public function setUnidadesResp($unidadesResp) {
            $this->unidadesResp = $unidadesResp;
            return $this;
        }
