...
class UGCron {
	...
	public static function add(UGCronJob $cronjob)
	{
		...
		if (!self::$object)
			throw new CHttpException('500', Yii::t('userGroupsModule.cron','you didn\'t initialize che UGCron singleton'));
	...
	public static function getStatus($cronName, $string = false, $print = false)
	{
		...
		if ($string) {
			...
			elseif ($status === self::NOT_INITIALIZED)
				$return_value = Yii::t('userGroupsModule.cron','you didn\'t initialize che UGCron singleton');
		} else
			$return_value = $status;
