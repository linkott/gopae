...
class UserGroupsInstallation extends CFormModel
{
	...
	public function authenticate($attribute,$params)
	{
		if (!Yii::app()->controller->module->accessCode)
			$this->addError('accesscode', Yii::t('userGroupsModule.install', 'You didn\'t set the accessCode in your config file.'));
		...
		if(!Yii::app()->getComponent('user') instanceof WebUserGroups)
			$this->addError('accesscode', Yii::t('userGroupsModule.install', 'You didn\'t correctly set the class property in the user component.<br/>Value setted: <b>{class}</b><br/>Value expected: <b>WebUserGroups</b>', array('{class}'=>get_class(Yii::app()->getComponent('user'))) ));
	}
