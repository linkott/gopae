...
class UserGroupsConfiguration extends CActiveRecord
{
	...
	public function tableName()
	{
		...
		return 'seguridad.usergroups_configuration';
	}
	...
	public function afterFind()
	{
		switch ($this->options) {
			case (!Yii::app()->user->accessRules === UserGroupsUser::ROOT_ACCESS && !isset(Yii::app()->user->accessRules['userGroups']['admin']['admin'])):
				...
				break;
			case 'CONST':
				//TODO when update will be implemented stop doing this
				if ($this->rule === 'version') {
					$this->render = UserGroupsInstallation::VERSION;
				} else
					$this->render = $this->value;
				break;
			case 'BOOL':
	...
	public static function findRule($rule)
	{
		...
		$model->scenario = 'find_rule';
                $cacheRule = Yii::app()->cache->get("rule$rule");
                if($cacheRule){
                    $result = $model->find($criteria);
                    Yii::app()->cache->set("rule$rule", $result, 86400);
                    if ($result !== NULL) {
