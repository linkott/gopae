...
class <?php echo $this->controllerClass; ?> extends <?php echo $this->baseControllerClass."\n"; ?>
{
    ...
    public static $_permissionControl = array(
        ...
        'label' => 'Módulo de <?php echo $this->controllerClass; ?>'
    );
    ...
    public function actionLista()
    {
        ...
        if($this->hasQuery('<?php echo $this->modelClass; ?>')){
            $model->attributes=$this->getQuery('<?php echo $this->modelClass; ?>');
        }
    ...
    public function actionRegistro()
    {
        ...
        if($this->hasPost('<?php echo $this->modelClass; ?>'))
        {
            $model->attributes=$this->getPost('<?php echo $this->modelClass; ?>');
            $model->beforeInsert();
            if($model->save()){
                $this->registerLog('ESCRITURA', 'modulo.<?php echo $this->modelClass; ?>.registro', 'EXITOSO', 'El Registro de los datos de <?php echo $this->modelClass; ?> se ha efectuado exitósamente. Data-> '.json_encode($model->attributes));
                Yii::app()->user->setFlash('success', 'El proceso de registro de los datos se ha efectuado exitosamente');
                $this->redirect(array('edicion','id'=>base64_encode($model-><?php echo $this->tableSchema->primaryKey; ?>),));
            }
        }
    ...
    public function actionEdicion($id)
    {
            ...
            if($this->hasPost('<?php echo $this->modelClass; ?>'))
            {
                ...
                $model->beforeUpdate();
                if($model->save()){
                    $this->registerLog('ACTUALIZACION', 'modulo.<?php echo $this->modelClass; ?>.edicion', 'EXITOSO', 'La Actualización de los datos de <?php echo $this->modelClass; ?> se ha efectuado exitósamente. Data-> '.json_encode($model->attributes));
    ...
    public function actionEliminacion($id)
    {
        ...
        if($model){
            $model->beforeDelete();
            if($model->save()){
                $this->registerLog('ELIMINACION', 'modulo.<?php echo $this->modelClass; ?>.eliminacion', 'EXITOSO', 'La Eliminación de los datos de <?php echo $this->modelClass; ?> se ha efectuado exitósamente. Data-> '.json_encode($model->attributes));
                if(Yii::app()->request->isAjaxRequest){
    ...
    public function actionActivacion($id){
        ...
        if($model){
            ...
            if($model->save()){
                $this->registerLog('ACTIVACION', 'modulo.<?php echo $this->modelClass; ?>.activacion', 'EXITOSO', 'La Activación de los datos de <?php echo $this->modelClass; ?> se ha efectuado exitósamente. Data-> '.json_encode($model->attributes));
                if(Yii::app()->request->isAjaxRequest){
    ...
    {
        if($this->hasPost('ajax') && $this->getPost('ajax')==='<?php echo $this->class2id($this->modelClass); ?>-form')
        {
    ...
    public function getActionButtons($data) {
        ...
        $columna .= CHtml::link("", "", array("class" => "fa icon-zoom-in", "title" => "Ver datos", 'href' => '<?php echo $baseUrl; ?>/consulta/id/'.$id)) . '&nbsp;&nbsp;';
        $columna .= CHtml::link("", "", array("class" => "fa icon-pencil green", "title" => "Editar datos", 'href' => '<?php echo $baseUrl; ?>/edicion/id/'.$id)) . '&nbsp;&nbsp;';
        if($data->estatus=='A'){
            $columna .= CHtml::link("", "", array("class" => "fa icon-trash red", "title" => "Eliminar", 'href' => '<?php echo $baseUrl; ?>/eliminacion/id/'.$id)) . '&nbsp;&nbsp;';
        }else{
            $columna .= CHtml::link("", "", array("class" => "fa fa-check green", "title" => "Activar", 'href' => '<?php echo $baseUrl; ?>/activacion/id/'.$id)) . '&nbsp;&nbsp;';
        }
        $columna .= '</div>';
        return $columna;
    }
