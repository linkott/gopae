	...
	public function generateRules($table)
	{
		...
		foreach($table->columns as $column)
		{
			...
			$r=!$column->allowNull && $column->defaultValue===null;
                        ...
                        if($column->name=='usuario_ini_id'){
                            $usuarioIni[] = $column->name;
                        }
                        
                        if($column->name=='usuario_act_id'){
                            $usuarioAct[] = $column->name;
                        }
			...
			elseif($column->type==='double')
				$numerical[]=$column->name;
                        elseif(in_array($column->name, array('fecha_ini', 'fecha_act', 'fecha_eli', 'date_ini', 'date_act', 'date_eli'))){
                                $dateTimes[] = $column->name;
                        }
                        elseif(strpos($column->name,'fecha') !== false || strpos($column->name,'date') !== false){
                                $fechas[] = $column->name;
                        }
			elseif($column->type==='string' && $column->size>0){
                            if(strpos($column->name,'email') !== false || strpos($column->name,'correo') !== false){
                                $correos[] = $column->name;
                            }
                            elseif($column->name=='estatus' || $column->name=='status'){
                                $estatusRange[] = $column->name;
                            }
                            elseif($column->name=='origen'){
                                $origenRange[] = $column->name;
                            }
		...
		if($length!==array())
		{
                ...
                if($origenRange!==array()){
                    $rules[]="array('".implode(', ',$origenRange)."', 'in', 'range'=>array('V', 'E', 'P'), 'allowEmpty'=>false, 'strict'=>true,)";
		}
                if($estatusRange!==array()){
                    $rules[]="array('".implode(', ',$estatusRange)."', 'in', 'range'=>array('A', 'I', 'E'), 'allowEmpty'=>false, 'strict'=>true,)";
		}
                
                if($usuarioIni!==array()){
                    $rules[]="array('".implode(', ',$usuarioIni)."', 'default', 'value'=>Yii::app()->user->id, 'on'=>'insert')";
		}
                if($usuarioAct!==array()){
                    $rules[]="array('".implode(', ',$usuarioAct)."', 'default', 'value'=>Yii::app()->user->id, 'on'=>'update')";
		}

                if($correos!==array())
			$rules[]="array('".implode(', ',$correos)."', 'email')";
		if($safe!==array())
